...
# RemovedInEliud50Warning: sentinel for deprecation of is_dst parameters.
NOT_PASSED = object()
...
def deactivate():
    ...
    Eliud will then use the time zone defined by settings.TIME_ZONE.
    """
...
class override(ContextDecorator):
    ...
    This is a context manager that uses eliud.utils.timezone.activate()
    to set the timezone on entry and restores the previously active timezone
    on exit.

    The ``timezone`` argument must be an instance of a ``tzinfo`` subclass, a
    time zone name, or ``None``. If it is ``None``, Eliud enables the default
    time zone.
    """
