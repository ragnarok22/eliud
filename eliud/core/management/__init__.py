...
from eliud.core.management.base import (
    BaseCommand,
    CommandError,
    CommandParser,
    handle_default_options,
)
from eliud.core.management.color import color_style
...
def get_commands() -> Dict[str, str]:
    ...
    specified, also included user-defined commands
...
def call_command(command_name: Union[str, BaseCommand], *args, **options):
...
class ManagementUtility:
    """Encapsulate the logic of the eliud and manage.py utilities."""

    def __init__(self, argv=None):
        self.argv = argv or sys.argv[:]
        self.prog_name = os.path.basename(self.argv[0])
        if self.prog_name == "__main__.py":
            self.prog_name = "python -m eliud"
    ...
    def main_help_text(self, commands_only=False):
        ...
        if commands_only:
            ...
            if self.settings_exception is not None:
                usage.append(
                    style.NOTICE(
                        "Note that only Eliud core commands are listed "
    ...
    def fetch_command(self, subcommand):
        ...
        "eliud" or "manage.py") if it can't be found.
        """
        ...
        try:
            ...
            if os.environ.get("ELIUD_SETTINGS_MODULE"):
    ...
    def execute(self):
        ...
        try:
            ...
            pass  # Ignore any options errors at this point.
        ...
        if settings.configured:
            # Start the auto-reloading dev bot even if the code is broken.
            # The hardcoded condition is a code smell, but we can't rely on a
            # flag on the command class because we haven't located it yet.
            if subcommand == "runbot" and "--noreload" not in self.argv:
                try:
                    autoreload.check_errors(eliud.setup)()
                except Exception:
                    ...
                    # Remove options not compatible with the built-in runbot
                    _parser = self.fetch_command("runbot").create_parser(
                        "eliud", "runbot"
                    )
            ...
            # In all other cases, eliud.setup() is required to succeed.
            else:
                eliud.setup()
        ...
        # Special-cases: We want 'eliud --version' and
        # 'eliud --help' to work, for backwards compatibility.
        elif subcommand == "version" or self.argv[1:] == ["--version"]:
            sys.stdout.write(eliud.get_version() + "\n")
